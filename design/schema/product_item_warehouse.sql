...
--   - denormalized path FKs into warehouse_warehouse (types: warehouse, zone, shelf, rack, bin)
--   - when warehouse_warehouse is re-parented or reordered, app must refresh this path —
...
CREATE TABLE product_item_warehouse (
    ...
    warehouse_id      BIGINT       NOT NULL REFERENCES warehouse_warehouse(id) ON DELETE RESTRICT, -- root type=warehouse
    zone_id           BIGINT       REFERENCES warehouse_warehouse(id) ON DELETE RESTRICT, -- type=zone
    shelf_id          BIGINT       REFERENCES warehouse_warehouse(id) ON DELETE RESTRICT, -- type=shelf
    rack_id           BIGINT       REFERENCES warehouse_warehouse(id) ON DELETE RESTRICT, -- type=rack
    bin_id            BIGINT       REFERENCES warehouse_warehouse(id) ON DELETE RESTRICT, -- type=bin (primary sync anchor)
