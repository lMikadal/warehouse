...
--   - each row = one order_order_item's price snapshot at the moment of this payment
--   - unique (order_payment_id, order_order_item_id): one snapshot per item per payment
CREATE TABLE order_payment_item (
    ...
    order_order_item_id   BIGINT        NOT NULL REFERENCES order_order_item(id) ON DELETE RESTRICT, -- source order line
    ...
    CONSTRAINT uq_order_payment_item_payment_order_item UNIQUE (order_payment_id, order_order_item_id)
);

CREATE INDEX idx_order_payment_item_payment    ON order_payment_item (order_payment_id);
CREATE INDEX idx_order_payment_item_order_item ON order_payment_item (order_order_item_id);
