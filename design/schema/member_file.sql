...
--   - display name from website_file.original_name (no local name column)
--   - purpose on website_file: 'member_document'
...
CREATE UNIQUE INDEX uq_member_file_user_file
    ON member_file (member_user_id, website_file_id)
...
CREATE INDEX idx_member_file_file        ON member_file (website_file_id);
