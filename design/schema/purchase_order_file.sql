...
--   - website_file_id replaces raw file TEXT column (files managed centrally)
--   - sort_order: gallery display order (lowest = cover)
--   - purpose on website_file: 'purchase_order_attachment'
...
CREATE UNIQUE INDEX uq_purchase_order_file_order_file
    ON purchase_order_file (purchase_order_id, website_file_id)
...
CREATE INDEX idx_purchase_order_file_file
    ON purchase_order_file (website_file_id);
