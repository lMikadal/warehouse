...
--   - image_url → website_file_id (purpose: purchase_order_payment_proof)
CREATE TABLE purchase_order_payment (
    ...
    website_file_id             BIGINT        REFERENCES website_file(id) ON DELETE RESTRICT, -- payment proof (purpose: purchase_order_payment_proof)
...
CREATE INDEX idx_purchase_order_payment_file       ON purchase_order_payment (website_file_id) WHERE website_file_id IS NOT NULL;
