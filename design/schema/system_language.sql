...
CREATE TABLE system_language (
    ...
    CONSTRAINT uq_system_language_locale  UNIQUE (locale),
    CONSTRAINT chk_system_language_locale CHECK  (locale ~ '^[a-z]{2,10}$')
);

-- at most one active default locale
CREATE UNIQUE INDEX uq_system_language_one_default
    ON system_language ((TRUE))
    WHERE is_default = TRUE AND deleted_at IS NULL AND is_active = TRUE;

CREATE INDEX idx_system_language_active_sort
    ON system_language (sort_order)
    WHERE deleted_at IS NULL AND is_active = TRUE;
CREATE INDEX idx_system_language_sort       ON system_language (sort_order) WHERE deleted_at IS NULL;
CREATE INDEX idx_system_language_created_by ON system_language (created_by)  WHERE created_by IS NOT NULL;
CREATE INDEX idx_system_language_updated_by ON system_language (updated_by)  WHERE updated_by IS NOT NULL;
