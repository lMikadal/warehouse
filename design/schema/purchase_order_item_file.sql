-- source: v1 order_purchase_items.image_url[] → normalized via website_file
--   - gallery for purchase_order_item
--   - sort_order: gallery display order (lowest = cover)
--   - purpose on website_file: 'purchase_order_item_image'
...
CREATE UNIQUE INDEX uq_purchase_order_item_file_item_file
    ON purchase_order_item_file (purchase_order_item_id, website_file_id)
...
CREATE INDEX idx_purchase_order_item_file_file
    ON purchase_order_item_file (website_file_id);
