import type { StoreSalesStatus } from "@/lib/order-store-api";
import { cn } from "@/lib/utils";

const pillBase =
  "inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium";

export function storeSalesStatusPillClass(
...
/** Transparent filter control; ring when a specific status tab is active. */
export function storeSalesStatusFilterButtonClass(
  status: StoreSalesStatus | "",
  active: boolean
): string {
  return cn(
