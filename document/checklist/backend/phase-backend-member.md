...
- [x] Register `/api/v1/member/*` in `cmd/server/main.go` + `CatalogPermPages` (settings, tiers, users)
- [x] Dev seed `seeds/dev/15_member_demo.sql`
- [x] Postman `Member` folder in `document/postman/postman.json`

## Required checklist

- [x] `make backend-test` — packages `member`, `rbac` (`CodeForRoute` for `/member/tiers`)
