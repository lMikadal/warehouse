...
- [ ] Manual: login `admin`/`admin` → expand ATW → zone cards read-only quotas; edit via zone sheet (active ≤ max); Management dialog or list warehouse view (`view?id=`) → all zones collapsed; zone card view (`view?id=&zone=`) → only that zone expanded; stat cards + legend + nested design tree items (L-connectors, one row per node); add/edit/delete shelf/rack/bin; drag grip on slots only — menu-style DnD via `move`; zone rows not draggable; nest into zone over quota → toast `warehouse.moveQuotaFull` (not raw API English)
