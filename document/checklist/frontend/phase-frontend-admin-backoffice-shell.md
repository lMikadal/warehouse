...
- [x] Admin roles / users CRUD ([`admin/roles`](../../../frontend/app/[locale]/(admin)/admin/(backoffice)/admin/roles/), [`admin/users`](../../../frontend/app/[locale]/(admin)/admin/(backoffice)/admin/users/) → BFF `/api/v1/auth/proxy/admin/*`, role permission matrix → `GET /v1/system/menus/permission-matrix`); segment `loading.tsx` column/toolbar props aligned with list chrome
