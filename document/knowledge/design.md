...
2. `js/seed/website_language.js` → **`js/seed/_admin_shared.js`** (must load before any seed file that calls `ADMIN_SEED_SHARED`) → `website_*` geo seeds → `admin_*.js` → `setting_*` seeds → `member_setting_*` seeds (incl. `member_setting_relation.js`) → `member_tier*` seeds → `index.js`
3. `js/store.js`
...
- **Product list**: [`product-list.html`](../../design/pages/product-list.html) + [`product-list.js`](../../design/js/pages/product-list.js) — one table row per **`product_item`** (variant), joined to `product_list` + languages; filters (search, category + brand selects, status/new button groups); table columns include sortable **category** and **brand** from `product_attribute`; pagination via shared `crudList.renderPaginationBar`; car fitment + warehouse modals from store; row edit/delete + status switch; edit/add → [`product-list-form.html`](../../design/pages/product-list-form.html) + [`product-list-form.js`](../../design/js/pages/product-list-form.js).
