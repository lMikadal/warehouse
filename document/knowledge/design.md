...
All uploaded files go through [`system_file`](../../design/schema/system_file.sql). Do **not** use `image_url TEXT` or `image_url[]` in new schema.

| Case | Pattern | Examples |
|------|---------|----------|
| Single image (logo, avatar, slip) | `system_file_id BIGINT` FK on parent table | `product_attribute`, `member_user`, `setting_bank` |
| Multiple files / reorder | junction `{entity}_file` + `sort_order` | `product_item_file`, `member_user_file`, `purchase_order_file` |

- Gallery cover = row with lowest `sort_order` (tie-break `id`)
- `ON DELETE RESTRICT` on `system_file_id` — cannot delete a file still referenced
- API resolves `system_file_id` → signed/public URL from `object_key` at read time

**`system_file.purpose` values:**

| Purpose | Used by |
|---------|---------|
| `product_attribute_logo` | `product_attribute.system_file_id` |
| `member_avatar` | `member_user.system_file_id` |
| `member_tier_badge` | `member_tier.system_file_id` |
| `setting_bank_logo` | `setting_bank.system_file_id` |
| `setting_sale_channel_logo` | `setting_sale_channel.system_file_id` |
| `purchase_order_payment_proof` | `purchase_order_payment.system_file_id` |
| `product_item_image` | `product_item_file` (gallery) |
| `member_document` | `member_user_file` |
