...
| Form UI | [`store-sales-form-page.tsx`](../../frontend/app/[locale]/(admin)/admin/(backoffice)/sales/store/_shared/store-sales-form-page.tsx) — step 1: [`RemoteComboboxField`](../../frontend/components/molecules/remote-combobox-field.tsx) member via [`store-sales-member-combobox.ts`](../../frontend/lib/store-sales-member-combobox.ts); payment radios from `member_setting_credit` (`loadMemberUserCreditOptions`); read-only address/tax from member detail; customer phases — **Next** locks step 1 and scrolls to `#store-sales-product-step`; **Change customer** opens price-warning dialog then unlocks customer while blocking product add/save until **Confirm customer** runs [`repriceStoreSalesCartLines`](../../frontend/lib/store-sales-cart-pricing.ts) (member/tier discounts on list price; wholesale when browse exposes it); print = `window.print()` only |
