...
| Approve layout | [`purchase-approve-page.tsx`](../../frontend/app/[locale]/(admin)/admin/(backoffice)/order/purchase/_shared/purchase-approve-page.tsx): tabs → `lg` two-column (`minmax(0,1fr)` + `340px`). Left: PO header meta + download, items table with **inline line edit** (SquarePen → qty/price/discount inputs + save/cancel), history timeline preview. Right (sticky): money summary (`PurchaseMoneySummary` emphasizeNet) + vertical approval stepper. |
| Payment layout | [`purchase-payment-page.tsx`](../../frontend/app/[locale]/(admin)/admin/(backoffice)/order/purchase/_shared/purchase-payment-page.tsx): same two-column chrome as approve (status current = pay). Right rail actions: **Confirm payment** (opens payment dialog) + Cancel / Request revision. Line edit via SquarePen when `paying` + item `approved`. |
| Detail layout | [`purchase-detail-page.tsx`](../../frontend/app/[locale]/(admin)/admin/(backoffice)/order/purchase/_shared/purchase-detail-page.tsx): same two-column chrome (read-only). Left: header + items (no actions) + history (+ payments/files when present). Right: money summary + approval stepper (steps through current status are green; no action buttons). Tabs include request detail when linked. |
| Sticky footer | Form page actions live in [`purchase-page-footer.tsx`](../../frontend/app/[locale]/(admin)/admin/(backoffice)/order/purchase/_shared/purchase-page-footer.tsx) (`fixed` bar + `useSidebar` inset + page `pb-20`); approve/payment/detail use the **right-rail** chrome — no sticky footer on those screens. |
