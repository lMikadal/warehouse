...
| Wired routes | `/admin/system/menu`, `/admin/system/permission`, `/admin/admin/roles`, `/admin/admin/users` (lists + sheets via BFF → Go `/admin/*` and permission matrix → `GET /system/menus/permission-matrix`); **setting:** `/admin/setting/{bank,vat,payment-method,sale-channel,code,claim-reason,prefix}` → BFF [`app/api/v1/auth/proxy/setting/*`](../../frontend/app/api/v1/auth/proxy/setting/) → Go `/api/v1/setting/*`; [`lib/setting-api.ts`](../../frontend/lib/setting-api.ts), shared lists [`setting/_shared/`](../../frontend/app/[locale]/(admin)/admin/(backoffice)/setting/_shared/) — **payment method** / **claim reason** ([`setting-lang-list.tsx`](../../frontend/app/[locale]/(admin)/admin/(backoffice)/setting/_shared/setting-lang-list.tsx)): boolean column filters via [`RemoteComboboxField`](../../frontend/components/molecules/remote-combobox-field.tsx) + `crud.filter.select` (`is_sale`/`is_purchase`, `is_claim`/`is_return`); inline `StatusSwitchField` on those columns + `is_active`; DnD off while bool filters active — **VAT** (`setting-vat-list` + `setting-vat-edit-sheet`): singleton table, edit-only, status switch in table + sheet (`is_active`), no search/pager/create/delete |
...
| Guard | [`middleware.ts`](../../frontend/middleware.ts) — `/admin/*` (except login) requires **access or refresh** cookie; neither → `/admin/login`; login page with either cookie → `warehouse_landing` or fallback. Sets request header `x-warehouse-admin-path` for SSR refresh return URL. Locale stripping / redirect prefix: [`lib/locale-path.ts`](../../frontend/lib/locale-path.ts). |
