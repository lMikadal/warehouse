...
| `GET/POST/PATCH/DELETE /product/lists/:id` | Aggregate graph (languages, codes, suppliers, cars, items with prices/suppliers/bin placements); **`GET`** includes root `updated_at` for admin form summary; create/update in one transaction; VAT rate snapshot from `setting_vat` on save |
