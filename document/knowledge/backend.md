...
**Purchase order (lot lookup):** migration `20260918180000_purchase_order_module.sql` — `purchase_order` (+ `vat_type` / `vat_rate` like stock), `purchase_order_item`, FK on `product_item_stock.purchase_order_item_id`. Dev demo: [`seeds/dev/14_purchase_order_demo.sql`](../../backend/internal/infra/postgres/seeds/dev/14_purchase_order_demo.sql) (runs before `14_product_demo.sql`). **Receive** (`POST /order/receives/:id/items/:itemId/receive`): catalog lines use existing `product_item_id`; **custom** lines create `product_list` + `product_item` (+ languages, optional supplier links) then stock lots — v1 `ReceiveNewItem` parity. One-bin-one-item: placement into a bin that already holds another item returns wrapped `ErrValidation` (`bin already holds another item`); `ticketError` surfaces the wrap detail (not only `invalid input`). **`GET /order/receives/bins?product_item_id=`** accepts `0` for free bins only (custom lines before catalog create).
| `GET /product/items/:id/history/purchase` · `…/history/sales` | Grouped purchase/sales history for one `product_item`. Query: `group_by=day\|month\|year` (+ `date_from`/`date_to` or `month_*` or `year_*`; default day = Bangkok YTD), optional `supplier_id` (purchase) / `customer` (sales = `order_list.member_name`), `sort_by=period` + `sort_order`. Purchase from `product_item_stock` (item×supplier children + `lot_count`); sales from `order_list_item` + `order_list` (excludes cancelled/rejected; net cost = weighted avg stock cost). Response `{ group_by, summary, groups[], meta }` |
...
| `GET /product/lists/:id/history/purchase` · `…/history/sales` | Same history as item routes, scoped to all variants under the list; optional `product_item_id` filter |
